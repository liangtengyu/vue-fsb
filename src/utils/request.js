...
// 统一配置
let GlobalRequest = axios.create({
...
// 拦截请求
GlobalRequest.interceptors.request.use((config) => {
...
// 拦截响应
GlobalRequest.interceptors.response.use((config) => {
  return config
}, (error) => {
  if (error.response) {
...
const request = {
  post (url, params) {
    return GlobalRequest.post(url, params, {
  ...
  put (url, params) {
    return GlobalRequest.put(url, params, {
      transformRequest: [(params) => {
        let result = ''
        Object.keys(params).forEach((key) => {
          if (!Object.is(params[key], undefined) && !Object.is(params[key], null)) {
  ...
  get (url, params) {
    ...
    return GlobalRequest.get(`${url}${_params}`)
  },
  delete (url, params) {
    let _params
    if (Object.is(params, undefined)) {
    ...
    return GlobalRequest.delete(`${url}${_params}`)
  },
  export (url, params = {}) {
